...
- `t_target`: a single forward pass of the target (Qwen3-8B by default) at
  several seq_lens, as a denominator.
...
Reports the full grid `(V, n, r) ↦ t_CP` plus `t_CP / t_target` at each
measured seq_len, and `t_CP / t_FF` for CP overhead relative to the
factorized baseline. Kill criterion: the (r=8, n=16, V=152064, L=2048)
ratio must be ≤ 0.3.
...
- `summary.json` — `target`, `ff`, `cp`, `ratio_grid`, `kill_criterion`
- `cp_latency.png` — CP time vs rank, one subplot per vocab
- `ratio_grid.png` — `t_CP / t_target` across the grid, with the
  kill threshold drawn in
